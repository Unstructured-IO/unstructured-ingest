...
# Get JWT token
jwt_token_resp=$(
	curl -sS -XPOST -H "Content-type: application/x-www-form-urlencoded" -d \
...
corpora_resp=$(
	curl -sS -L -X POST 'https://api.vectara.io/v1/list-corpora' \
	-H 'Content-Type: application/json' \
	-H 'Accept: application/json' \
	-H "customer-id: $VECTARA_CUSTOMER_ID" \
	-H "Authorization: Bearer $access_token" \
...
get_corpus_size=$(
	curl -L -X POST 'https://api.vectara.io/v1/compute-corpus-size' \
	-H 'Content-Type: application/json' \
	-H 'Accept: application/json' \
	-H "customer-id: $VECTARA_CUSTOMER_ID" \
	-H "Authorization: Bearer $access_token" \
