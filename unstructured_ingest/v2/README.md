...
To add a source connector, refer to [local.py](unstructured_ingest/v2/processes/connectors/local.py) as an example that implements the the uploader abstract base classes with the associated configs.
...
- [ ] Create a new file under [connectors/](unstructured_ingest/v2/processes/connectors/) implementing the the base classes required depending on if it's a new source or destination connector.
