...
class KafkaAccessConfig(AccessConfig):
    kafka_api_key: t.Optional[str] = Field(description = "Kafka API key to connect at the server")
    secret: t.Optional[str] = Field(description = "")
...
class KafkaIndexer(Indexer):
    ...
    def create_consumer(self) -> "Consumer":
        ...
        is_confluent = self.connection_config.confluent
        bootstrap = self.connection_config.bootstrap_server
...
class KafkaDownloader(Downloader):
    connection_config = KafkaConnectionConfig
    download_config = KafkaDownloaderConfig = field(
        default_factory = lambda: KafkaDownloaderConfig
    )
    ...
    def _tmp_download_file(self):
        topic_file = self.connection_config.topic + "-" + self.raw_filename
    ...
    @SourceConnectionError.wrap
    def run(self, file_data: FileData, **kwargs: t.Any) -> download_responses:
        self._get_file()
