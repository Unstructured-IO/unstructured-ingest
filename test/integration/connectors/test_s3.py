...
@pytest.mark.tags(CONNECTOR_TYPE, DESTINATION_TAG, BLOB_STORAGE_TAG)
@requires_env("S3_INGEST_TEST_ACCESS_KEY", "S3_INGEST_TEST_SECRET_KEY")
async def test_s3_destination_file_already_exist(upload_file: Path):
    aws_credentials = get_aws_credentials()
    s3_bucket = "s3://utic-ingest-test-fixtures"
    # destination path already contains file with same name. file contents are different though.
    destination_path = f"{s3_bucket}/destination/ingest-test-dnc/"
    connection_config = S3ConnectionConfig(
        access_config=S3AccessConfig(
            key=aws_credentials["aws_access_key_id"],
            secret=aws_credentials["aws_secret_access_key"],
    ...
    try:
        ...
        if uploader.is_async():
            ...
            uploader.run(path=upload_file, file_data=file_data)
        uploaded_files = [
            Path(file) for file in s3fs.ls(path=destination_path) if Path(file).suffix == ".json"
        ]

        assert len(uploaded_files) == 2
    finally:
        s3fs.rm(
            path=destination_path + "DA-1p-with-duplicate-pages.pdf.json(1).json", recursive=False
        )
