...
async def validate_upload(client: Redis, upload_file: Path):
    ...
    embedding_similarity = np.linalg.norm(np.array(response['embeddings']) - np.array(expected_embeddings))
...
async def redis_destination_test(upload_file: Path, tmp_path: Path, connection_kwargs: dict, uri: Optional[str] = None, password: Optional[str] = None):
    stager = RedisUploadStager(
        upload_stager_config=RedisUploadStagerConfig(),
    )
    uploader = RedisUploader(
        connection_config=RedisConnectionConfig(**connection_kwargs, access_config=RedisAccessConfig(uri=uri, password=password)),
...
async def test_redis_destination_azure_with_password(upload_file: Path, tmp_path: Path):
    connection_kwargs = {'host':'utic-dashboard-dev.redis.cache.windows.net', 'port':6380, 'db':0, 'ssl':True}
    password = redis_pw
