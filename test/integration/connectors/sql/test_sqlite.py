...
async def test_sqlite_destination(upload_file: Path, temp_dir: Path, db_path: Path):
    ...
    uploader = SQLiteUploader(connection_config=SQLiteConnectionConfig(database_path=db_path))
    uploader.run(path=staged_path, file_data=mock_file_data)

    with staged_path.open("r") as f:
        staged_data = json.load(f)
    validate_destination(db_path=db_path, expected_num_elements=len(staged_data))
