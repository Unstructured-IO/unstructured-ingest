...
def test_conform_dict(monkeypatch, test_element_dict, metadata_fields, expected_to_exist,
                      not_expected_to_exist):
    if metadata_fields is not None:
        stager = PineconeUploadStager(upload_stager_config=PineconeUploadStagerConfig(metadata_fields=metadata_fields))
    else:
    ...
    assert all(results["metadata"][key] == test_element_dict["metadata"][key] for key in expected_to_exist)
