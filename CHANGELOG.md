...
- **fix(error): correct `UserError.status_code` from `401` to `422`.** `401` is HTTP "Unauthorized" (unauthenticated) which is what `UserAuthError` covers. `UserError` is for invalid user input / config, which is HTTP `422` (Unprocessable Entity). Subclasses that override `status_code` (`UserAuthError=401`, `RateLimitError=429`) are unaffected; `QuotaError` (which inherits without override) now reports `422` — flag for callers depending on the prior `401`.
