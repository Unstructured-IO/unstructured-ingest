...
- **fix(teradata): refuse a destination credential that cannot create the table it would auto-create.** When the configured table does not exist yet, or no table is configured, the Teradata uploader creates it at upload time with `create_destination()`, which runs the connector's own DDL asset in the session's database. The destination precheck never asked for the rights that CREATE needs, so a user without them passed the connector check and the job then failed creating the table, reaching the customer as "Failed to connect to server". With the Database field blank the table lands in whatever database the session defaults to, and nothing reported which one. Precheck now resolves that database with `SELECT DATABASE` (the configured one when set, else the session default, spelled as the server stores it), logs it, and runs `create_destination()`'s own DDL under a throwaway `unstructured_precheck_<hex>` name and drops it again, unless the configured table already exists there. The probe runs the real statement, columns, types and `PRIMARY INDEX` and all, rather than a one-column stand-in, because a narrower probe passes a credential the real CREATE refuses; it is unqualified, the way `create_destination()` runs it, so it lands in the same session database and no identifier is interpolated into the SQL. Teradata 3524 and 3523 refuse the destination with a `UserError` naming the database. Neither code names the right it refused, they differ in what was refused: 3524 is a right on a database, which on a CREATE TABLE is CREATE TABLE, and 3523 is a right on an object inside one, which is how a table with a `VECTOR32` column being refused UDTUSAGE on `SYSUDTLIB` arrives. The 3523 message says so rather than telling the customer CREATE TABLE is what they are missing. With the Database field blank the message also says so, because the database it names is only the session default and granting rights on it is not necessarily what the customer wants. Any other failure is logged and passes, including a failure tearing the session down, except that a refusal the server already gave is no longer dropped by one. A probe table whose DROP fails is named in a warning rather than failing the check. A configured table that already exists is unchanged: it gets the zero-row INSERT/DELETE probe from 1.11.16.
- **A destination with no table configured can now be refused where it used to pass.** The probe always runs in that case, because the caller names the table only when it calls `create_destination()`, so precheck has nothing to look up. A user whose per-workflow table already exists but who has since lost the right to create one is therefore refused by the connector check, even though the job would have written to the existing table. This is not only the connector's Test Connection button: `Pipeline.run()` prechecks every step of every run in this library, and in the platform the preflight gate prechecks at the start of every job wherever it is enabled.
- **Source-side behaviour change.** 3524 joins `_USER_FAULT_TERADATA_CODES`, which is consulted at every classification site in this connector and not only on the destination. On the upload path it stops surfacing as `DestinationConnectionError` with the catch-all "Failed to connect to server {host}" and becomes `UserError` (422). On the source path `TeradataIndexer.precheck`'s `SELECT TOP 1` probe now raises `UserError` rather than `SourceConnectionError` on 3524, so source callers catching `SourceConnectionError` no longer catch that case. On a SELECT the code is unlikely, but the change is the same one 1.11.14 made for the value-rejection codes.
