...
- **fix(teradata): refuse a destination table the credential cannot read.** A table an admin had already created, with the connector's own user holding nothing on it, passed the destination check and then failed every record of the job. When the configured table is already in the data dictionary, `create_destination()` builds nothing and the job's first act on that table is to read it, so the check now runs that read and refuses a credential the server turns away on the codes that mean "no privilege" and nothing else (3523, 3524, 5315, 5612). On a live Vantage 20.0.0.68 that read answers 3523 and names the table, which is the path this case takes. `DBC.TablesV` is the non-restricted form of that view and returns every object regardless of ownership or privilege, where `DBC.TablesVX` returns only what the user owns, created, was granted, or reaches through a role, so a row in it is an existence fact that does not depend on this credential's rights. With no row in the dictionary nothing changed: the table is not there yet, and the CREATE TABLE probe still owns that case.
- **3807 refuses nothing, at any site, even when the dictionary has just listed the table.** An admin can drop the table between the dictionary lookup and the read, and a destination whose table is absent works, because `create_destination()` builds it. Treating "the dictionary says it exists" plus "the read says it does not" as a denial would convert that race into the one outcome this check must never produce: a destination that works, reported as broken. No measured server answers 3807 for the no-privilege case, so the code gives up nothing by passing on it.
- **A read the server refused reached nobody before.** This connector's `get_table_columns()` re-raises the driver error as a typed `UserError`, and the shared write check classifies the exception it is handed by asking whether it came from the driver, which that wrapper is not, so every schema-read denial on this dialect was swallowed. The probe that now runs before it reads the driver's own answer.
- **The existence lookup now covers a table with no primary index.** Teradata files a NoPI table under `TableKind 'O'`, not `'T'`, so a destination an admin created without a primary index read as absent: `create_destination()` would try to build its own on top of it and the check above would never run. The predicate is `TableKind IN ('T','O')`. Kinds that are not base tables stay out, because calling a view or a queue table "the destination already exists" would hand the upload an object it cannot write instead of the 3803 it raises today.
