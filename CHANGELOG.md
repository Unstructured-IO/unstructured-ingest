...
- **fix(databricks-volumes): make the destination connection check read the configured volume.** The Volumes uploader `precheck` asserted `current_user.me().active` and never read the configured path, so a volume that does not exist, a path typo, a principal with no `WRITE VOLUME` grant, and a catalog or schema the principal cannot `USE` all passed the connection test and failed at `files.upload` mid-job, after every file had already been partitioned, enriched and embedded. The precheck now asks Unity Catalog about the destination instead: `volumes.read` on the three-level name, `grants.get_effective` for this principal's privileges on it, and `get_directory_metadata` on the configured `volume_path`. Nothing is written to the volume and nothing is removed from it, because a volume is routinely watched -- an Auto Loader stream or a file-arrival trigger without a glob filter cannot tell a zero-byte probe file from real input. One thing fails the check: Unity Catalog reporting that the volume does not exist, and only when the grant lookup did not already resolve the same volume. Everything else is logged and allowed through, so no destination that works today starts failing. `WRITE VOLUME` missing from the grant answer is a warning naming the volume and the privilege, not a refusal, because the effective-permissions API is documented to expand privileges inherited from parent securables and says nothing about expanding group membership; for the same reason a 403 on any of the reads is a warning, since `WRITE VOLUME` does not imply `READ VOLUME`. The failure names the volume and the configured path, which is the one thing the old check hid.
- **Databricks Volumes errors are now classified by their HTTP status even when the SDK raises a more specific subclass, and `ABORTED` is treated as retryable.** The SDK raises the error-code class in preference to the status class, so a 404 carrying `RESOURCE_DOES_NOT_EXIST` arrives as `ResourceDoesNotExist`, a `NotFound` subclass. `wrap_error` looked the exception type up by exact match, so these subclasses (`ResourceDoesNotExist`, `InvalidParameterValue`, `RequestLimitExceeded`, `ResourceExhausted`, `DataLoss` and the rest of the error-code mapping) fell through as unhandled and were re-raised raw. They now map like their parent status: `UserError` for a 4xx, `RateLimitError` for a 429, `ProviderError` for a 5xx. `Aborted` is the one exception to that rule: it is a 409, but it means Databricks lost a race with itself -- two writers on one path, a sequencer check that did not hold -- so classifying it with the rest of the 4xx made it terminal and dropped a write that retrying would have cleared. It is classified as a `ProviderError`, which stays retryable. This applies to every Databricks Volumes connector (native, AWS, Azure, GCP) at precheck and at run time on both source and destination, and it is what lets the destination check fail on a missing volume instead of letting it through.

## [1.11.20]

### Fixes

- **fix(teradata): probe the CREATE TABLE right with the real destination DDL, not a one-column stand-in.** 1.11.18's precheck probe was `CREATE TABLE <throwaway> (probe_col INTEGER)`, which asks the server for less than `create_destination()` will: the connector's own DDL asset carries `CLOB`, `VECTOR32`, `JSON` and a `PRIMARY INDEX`, and a right that only those types need was invisible to the check and reached the customer at upload instead. The probe now runs `create_destination()`'s own statement through the shared `_elements_schema_sql()` under the throwaway name, so the rights it asks for are the rights the upload needs: no wider, no narrower, which is the rule 1.11.16 set for every dialect. It runs unqualified, the way `create_destination()` runs it, so it lands in the session database the check just resolved and that database name never reaches the SQL text.
- **Teradata 3523 now refuses the destination alongside 3524.** The probe is the real statement, so an answer meaning "a right was refused and nothing else" is an answer about the real CREATE, and a right the column types need comes back as 3523 rather than 3524 (a `VECTOR32` column refused UDTUSAGE on `SYSUDTLIB` arrives that way). The two codes differ in the granularity of the object refused, not in naming the right: 3524 refuses a right on a database, which on a CREATE TABLE is CREATE TABLE, and 3523 refuses a right on an object inside one. The 3523 message therefore does not tell the customer CREATE TABLE is what they are missing, because on that code it is not.
- **With the Database field blank, both refusal messages now say so.** The database they name is only the session default, so a DBA who follows the message otherwise grants rights on a database nobody chose, which is the complaint this check came from. The message now also points at the Database field.
- **With no Table Name configured, the refusal now names the field that settles it.** That is the one case this check can refuse a credential the job would not have needed: the probe always runs when no table is configured, because the caller names the table only when it calls `create_destination()`, so a user whose table already exists and who has since lost CREATE TABLE is refused even though the upload would have found that table and returned early without creating one. The refusal now says so and points at the Table Name field, which makes the check look the table up and skip the CREATE probe entirely. The refusal is kept rather than downgraded to a warning because that is the path the platform uses, so warning there would switch the check off where it is meant to run.
- **A refusal the server already gave is no longer dropped on the way out of the block.** `denied` is set before the `try`, so a cursor close or the connection's own commit raising after the CREATE came back refused no longer turns a confirmed refusal into "inconclusive" and passes the credential.
- **Logging.** The probe table is named before it is created, so a process killed between the CREATE and the DROP leaves a traceable name; `create_destination()` now names the database it is creating in, which only precheck used to report; and each precheck outcome (skipped, refused, created, inconclusive) has its own line, with inconclusive at `info` to match the shared `_run_write_probe`.
- **Completing what 1.11.18's entry did not say.** Two behaviour changes shipped in 1.11.18 and were not written down. First, a destination with **no table configured** can now be refused where it used to pass: the probe always runs in that case, because the caller names the table only when it calls `create_destination()`, so precheck has nothing to look up, and a user whose per-workflow table already exists but who has since lost the right to create one is refused by the connector check even though the job would have written to the existing table. Second, **3524 was reclassified on the source side too**: it joined `_USER_FAULT_TERADATA_CODES`, which is consulted at every classification site in this connector, so `TeradataIndexer.precheck`'s `SELECT TOP 1` probe now raises `UserError` rather than `SourceConnectionError` on 3524. On a SELECT that code is unlikely, but source callers catching `SourceConnectionError` no longer catch it.
...
- **fix(sql): refuse a destination credential that can connect and read but cannot write.** The shared SQL destination precheck was `SELECT 1`, which runs against the session and so says nothing about the table the uploader writes to: a credential holding CONNECT, USAGE and SELECT and no INSERT passed the connection check and then failed on every record at write time, with the permission problem surfacing as an upload failure halfway through a job instead of at setup. Precheck now also asks the question it was meant to ask, using the real statements made harmless. `upload_dataframe` is delete-then-insert, so both are asked and the probe's privilege surface matches the upload's exactly -- wider refuses a credential that works, narrower passes one that cannot write: `INSERT INTO <table> (<columns>) SELECT <columns> FROM <table> WHERE 1 = 0`, and `DELETE FROM <table> WHERE 1 = 0` behind the same `can_delete()` gate the upload puts its DELETE behind, so a table with no record-id column is never asked for a right the upload does not use. The engine runs the privilege check when it plans the statement, before any row is produced, so a refusal arrives while the row count is still zero and nothing is written or removed even on the drivers that are in autocommit. `<columns>` is the uploader's own INSERT column list (`get_table_columns()`, which `_fit_to_schema` also conforms the frame to), not `*`, because grants are per-column on every engine here and asking for rights the write path does not use would refuse credentials that work. The check is one-sided by construction: it refuses only on a driver answer the dialect recognizes as an unambiguous privilege denial -- PostgreSQL `42501` and `25006`, SingleStore `1044`/`1142`/`1143`, Snowflake's access-control error, Teradata `3523`/`5315`/`5612`, and a read-only database file on SQLite -- and passes on everything else, including a timeout, a dropped connection, a missing table, a schema error, an unrecognized code, or a dialect with no classifier. Codes that mean "does not exist OR you may not see it" (PostgreSQL `42P01`, Teradata `3807`, Snowflake `002003`) are deliberately not treated as denials, so a typo'd table name is never reported as a permissions problem. The schema read the probe builds its column list from is classified the same way, so a credential with no rights at all -- which fails that read rather than the write -- is refused by name instead of passing as "schema unavailable" and failing a job later; Teradata is the exception, where the table not existing yet is a working destination because `create_destination()` builds it at upload time. A refusal is a `UserError` (422) naming the table and the specific right that was refused -- a credential that can insert but not delete is told it needs DELETE, not INSERT -- and both probes run even when the first is refused, so a credential short of both is told both at once. It is never a `UserAuthError`: the credential is valid, and telling the customer to rotate it sends them to fix the wrong thing. Covers postgres, singlestore, sqlite, snowflake and teradata; databricks-delta-tables, vastdb and ibm-watsonx-s3 define their own precheck and are unchanged. Two things stay invisible to a zero-row statement and still fail at upload: a rule evaluated per row (a PostgreSQL row-level-security `WITH CHECK` policy, a Snowflake row access policy), and the CREATE TABLE right Teradata's auto-create path needs when the configured table does not exist yet.
...
- **feat(fsspec, filter): report why a source indexed nothing, instead of only how little it indexed.** A run that moved zero files could not be told apart from a run whose source was empty: `FsspecIndexer.get_file_info()` applied `size > 0 and type == "file"` in the same expression that consumed the client listing, so the pre-filter count was never held, and the glob/size `Filterer` counted nothing at all. Three different causes -- an empty or wrong path, a path holding only directory markers and zero-byte keys, and `file_glob`/`max_file_size` rejecting everything -- all ended as `0 documents`. The indexer now keeps the listing count and reports it alongside the surviving count as `source.listing.returned` / `source.listing.retained`, and the filter step reports `filter.<stage>.received` / `filter.<stage>.retained` per stage (`indexed`, `downloaded`, `uncompressed`, so a file surviving every pass is not counted three times). Both go on the OTel span the work already runs inside -- the channel the platform already points somewhere via `ProcessorConfig.otel_endpoint` / `OTEL_EXPORTER_OTLP_ENDPOINT` -- and each zero case also emits one consolidated `WARNING` naming which zero it was. Query both families on the root `ingest process` span, not on `indexer` or `filter step`: `IndexStep.run` is a generator function under `@instrument`, so its span opens and closes before the body ever runs, and `FilterStep` reports after the instrumented `__call__` has already returned. Reporting only: nothing new is raised, which matters in a base class every fsspec connector inherits. Read `source.listing.retained` with one caveat: it is counted before `sample_n_files` is applied, so on a sampled run it is larger than the number of files actually indexed.

### Fixes

- **fix(pipeline): an uncompress step that extracts nothing no longer crashes the run.** `clean_results` returns `None` rather than `[]` when everything it is handed is falsy. The post-download path guards for that; the post-uncompress path handed the `None` straight to `apply_filter`, which iterated it and died with `TypeError: 'NoneType' object is not iterable`. It needed an uncompress step, a configured filterer, and every archive extracting to zero regular files, which is exactly the everything-got-dropped case these diagnostics exist to name. Guarded at the call site so the exit is reported as an empty uncompress, not as a filter that dropped everything.

## [1.11.7]

### Fixes

- **fix(fsspec): classify a rejected credential raised while listing, instead of letting it escape unclassified.** `precheck()` wrapped its listing through `wrap_error`; `get_file_info()`, the path a real job takes, did not, so fsspec's bare `PermissionError` (rejected key) and `FileNotFoundError` (missing prefix) escaped without a `status_code`. Downstream that made a source whose credentials were rejected indistinguishable from one that was legitimately empty: measured on an SND, a source over a bucket holding three files returned 0 documents and a SUCCESS verdict with bad credentials, while the same bucket with good credentials returned 3. The listing is now wrapped the same way `precheck()` wraps it, so S3 maps the rejection to `UserAuthError`. Post-listing filtering stays outside the try, so a malformed entry is still reported as our own bug rather than a connection failure. Inherited by all six fsspec-backed connectors (s3, gcs, azure, box, dropbox, sftp).
