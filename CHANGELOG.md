...
- **fix(teradata): probe the CREATE TABLE right with the real destination DDL, not a one-column stand-in.** 1.11.18's precheck probe was `CREATE TABLE <throwaway> (probe_col INTEGER)`, which asks the server for less than `create_destination()` will: the connector's own DDL asset carries `CLOB`, `VECTOR32`, `JSON` and a `PRIMARY INDEX`, and a right that only those types need was invisible to the check and reached the customer at upload instead. The probe now runs `create_destination()`'s own statement through the shared `_elements_schema_sql()` under the throwaway name, so the rights it asks for are the rights the upload needs: no wider, no narrower, which is the rule 1.11.16 set for every dialect. It runs unqualified, the way `create_destination()` runs it, so it lands in the session database the check just resolved and no identifier reaches the SQL text.
